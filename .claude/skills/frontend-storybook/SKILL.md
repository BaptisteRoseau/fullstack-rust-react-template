...
description: Use this skill when writing or editing a storybook component.
---
...
| Language | TypeScript |
| Package Manager | Bun |
| Addons | @storybook/addon-links, storybook/internal/node-logger, @storybook/addon-docs, @storybook/addon-a11y |
...
6. **Use `Bun` for every install** (detected from this project's lockfile).
...
Use `msw-storybook-addon`. Install with:
    ...
    ```bash
    bun add -D msw msw-storybook-addon mockdate
...
Then run the project's TypeScript check (use the script from `package.json` — typically `tsc --noEmit` or `bun run typecheck`). Read the raw output once; don't pipe it through repeated `grep`/`head` invocations to slice it.
