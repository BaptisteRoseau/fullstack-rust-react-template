...
// Backend-for-Frontend half of the Authenticator trait suite: the Authorization
// Code + PKCE flow. See `authenticator.rs` for the conventions and the macro
// every test here must be registered in.

/// Where the user asked to land once the login completes.
const POST_LOGIN_REDIRECT: &str = "/app/dashboard";

pub async fn assert_authorize_url_targets_the_login_screen(
...
pub async fn assert_authorize_url_targets_the_registration_screen(
    authenticator: &impl Authenticator,
) {
    let url = authenticator
...
pub async fn assert_exchange_code_returns_tokens_and_redirect(
    authenticator: &impl Authenticator,
...
pub async fn assert_exchange_code_issues_a_token_that_validates(
    authenticator: &impl Authenticator,
    agent: &impl ProviderAgent,
) {
    ...
    let want_id = Uuid::parse_str(BFF_USER_ID).expect("BFF_USER_ID must be a valid uuid");
    assert_eq!(
        user.id, want_id,
...
pub async fn assert_exchange_code_rejects_an_unknown_state(
    authenticator: &impl Authenticator,
) {
...
pub async fn assert_exchange_code_rejects_a_replayed_state(
    authenticator: &impl Authenticator,
    agent: &impl ProviderAgent,
) {
...
pub async fn assert_userinfo_returns_the_identity_claims(
    authenticator: &impl Authenticator,
    agent: &impl ProviderAgent,
) {
    let session = log_in(authenticator, agent, None).await;
...
pub async fn assert_refresh_tokens_issues_a_new_pair(
    authenticator: &impl Authenticator,
    agent: &impl ProviderAgent,
) {
    let session = log_in(authenticator, agent, None).await;
...
pub async fn assert_logout_revokes_the_session(
    authenticator: &impl Authenticator,
    agent: &impl ProviderAgent,
) {
    let session = log_in(authenticator, agent, None).await;
